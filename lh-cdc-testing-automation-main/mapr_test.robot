...
Create Volume Flavor
  ...
  Create Volume Flavor Yaml Attributes  ${PORTAL_ID}  ${hoster_id}  ${flavor_id}  ${name}  ${latency_value}  ${3}  high_throughput  ${EMPTY}  ${EMPTY}  ${EMPTY}  ${EMPTY}  ${FLAVOR_YAML} 
...
Verify User Ticket With Retry
  Wait Until Keyword Succeeds    5x    10 sec  Get User Ticket For Attachment  volume_attachment_id=${volume_attachment_id} 
